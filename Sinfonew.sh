...
sed -i '$d' schema.txt
sed -i "1d" schema.txt
echo "Esquema|Creacion|Nombre|Apellido|Telefono|Rut_usuario|Email|Rut_Empresa|Rol" >>  respuestas.txt

while read schema 
do
PGPASSWORD='PCmSCF9QVy99A4EW' psql -X -o $schema.txt -U angel_gongora -h pg-production-read.portalfinance.io --set ON_ERROR_STOP=on pf_production_service_core_db  <<SQL
select b.created_at, initcap(b.first_name) as "Nombre", initcap(b.last_name) as "Apellido", 
b.phone_number as "Telefono" , b.tax_number as "Rut_usuario", a.email as "Email", 
c.tax_number as "Rut_Empresa"
